...
import com.sailmi.erp.model.sso.Form;
				...
				String[] dir=new File(path).list();
					for(String f:dir){
						if(!"code".equals(f)&&!"modelList".equals(f)&&!"html".equals(f)&&!f.endsWith(".html")&&!"sql".equals(f)){
							...
							temp.process(p, out);
							File dir_java=new File(path+"/code/net/joffee/"+p.get("package")+"/"+f.toLowerCase());
							dir_java.mkdirs();
							File tf=new File(path+"/code/net/joffee/"+p.get("package")+"/"+f.toLowerCase()+"/"+p.get("Model")+f.replace("model", "")+".java");
