 ...
 * @author 歌樵
 */
@TableBind(name="crm_contact_record")
public class ConcatRecord extends Model<ConcatRecord> {
	private static final long serialVersionUID = 8699093530520166772L;
	public static final String tableName="crm_contact_record";
	public static ConcatRecord dao=new ConcatRecord();
	public Page<ConcatRecord> page(int pageNo, int pageSize, Map<String, Object> filter,Integer qryType) {
		...
		sql.append(Business.tableName);
		sql.append(" b on t.business_id=b.id ");
		...
		if(StringUtils.isNotEmpty(start_date)){
			sql.append(" and (t.create_datetime >= ? or t.concat_datetime>=?)");
		...
		return dao.paginate(pageNo, pageSize, "select t.id,t.subject,t.concat_datetime,t.concat_type,t.sale_stage,t.cust_status,t.create_datetime,b.name business_name,t.next_datetime,ct.name contacts_name,c.realname as creater_name,cust.name customer_name,cust.sn ",sql.toString(),parame.toArray());
	}
	public ConcatRecord findById(String id,String company_id){
		String userView=Person.tableName;
		StringBuffer sql=new StringBuffer("select t.*,ct.name contacts_name,c.realname as creater_name,cust.name customer_name,cust.sn,b.name business_name from ");
		sql.append(tableName);
		sql.append(" t left join ");
		sql.append(Customer.tableName);
		sql.append(" cust on cust.id=t.customer_id left join ");
